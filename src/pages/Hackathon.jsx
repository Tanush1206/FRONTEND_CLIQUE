  ...
  useEffect(() => {
    const fetchHackathonEvents = async () => {
      try {
        ...
        console.error('Error fetching hackathon events:', err);
    ...
    const interval = setInterval(fetchHackathonEvents, 30000);
              ...
              {loading ? (
                ...
                events.map((event) => (
                  <EventCard
                    ...
                    eventType="Hackathon"
                    color="green"
                    statusText={event.statusText || 'Join us for an exciting hackathon'}
                  />
                ))
              ) : (
                <div className="text-white/80 text-center py-8">
