...
const Fest = () => {
  ...
  useEffect(() => {
    const fetchFestEvents = async () => {
      try {
        setLoading(true);
        const response = await fetch('http://localhost:4000/api/events?category=fest&limit=50');
    ...
    const interval = setInterval(fetchFestEvents, 30000);
              ...
              {loading ? (
                ...
                events.map((event) => (
                  <EventCard
                    ...
                    eventType="Fest"
                    color="orange"
                    statusText={event.statusText || 'Join us for an amazing fest experience'}
                  />
                ))
              ) : (
                <div className="text-white/80 text-center py-8">
                  <p>No fest events found</p>
